...
# added by Andy Park
function void setJointPositions(string limb, double[] command)
function void requestJointPositions()
end object
...
class Baxter_impl(object):
	...
	# set joint positions upon request modified by Andy
    def setJointPositions(self, limb, command):
        limb = limb.lower()
        if not limb in self._valid_limb_names.keys():
            return
    ...
    # read joint positions upon request modified by Andy
    def requestJointPositions(self):
        self.readJointPositions()
    ...
    def command_worker(self):
        while self._running:
            t1 = time.time()
                ...
                #self._left.set_joint_positions(self._l_joint_command)
                #self._right.set_joint_positions(self._r_joint_command)
            if (self._mode == self.MODE_VELOCITY):
